...
void initialize(int height, int width, float *kernel1, float *kernel2, int ksize, float threshold = -1, int batchSize = 1)
{
    ...
    cudaMalloc(&d_temp, sizeof(float2) * img_size * batchSize);
...
{
    size_t img_size = width * height;
    size_t total_size = img_size * batchSize;
    ...
    for (int i = 0; i < batchSize; ++i)
    {
        ...
        cudaStreamDestroy(streams[i]); // clean up
    }

    // cudaDeviceSynchronize(); // wait for all async ops
